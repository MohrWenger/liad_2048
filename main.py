...
class Game(tk.Frame):
    ...
    def GUI_maker(self):
        ...
        for i in range(4):
            row = []
            for j in range(4):
                frame_cells = tk.Frame(self.grid_main,
        ...
        tk.Label(frame_score, text="Score",
                 font=Game.Font_ScoreLabel).grid(row=0)
        self.label_score = tk.Label(frame_score,
                                    text="0",
    ...
    def start_game(self):
        ...
        self.cells[row][col]["number"].configure(bg=Game.Color_Cells[2],
    ...
    def combine(self):
        for i in range(4):
            for j in range(3):
                if self.matrix[i][j] != 0 and self.matrix[i][j] == self.matrix[
                        i][j + 1]:
                    self.matrix[i][j] *= 2
                    self.matrix[i][j + 1] = 0
    ...
    def game_over(self):
        if any(2048 in row for row in self.matrix):
            ...
            tk.Label(game_over_frame,
                     ...
                     font=Game.Font_GameOver).pack()
        elif not any(
                0 in row
                for row in self.matrix) and not self.Exists_horizontalMoves(
                ) and not self.Exists_verticalMoves():
            game_over_frame = tk.Frame(self.grid_main, borderwidth=2)
            game_over_frame.place(relx=0.5, rely=0.5, anchor="center")
            tk.Label(game_over_frame,
                     text="GAME OVER!!",
